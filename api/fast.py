...
app.state.model = SentenceTransformer('models/model_clip')
#Style classification model
app.state.model_keras = models.load_model("models/model_Xception_alldata_finetuned.keras")
#Database connexion with all embeddings of the artwork dataset
...
@app.get("/")
def index():
    """This function is a test for the API"""
    return {"status": "ok CLIP"}


@app.post('/upload_image')
async def receive_image(img: UploadFile=File(...)):
    """This function will return the 5 most similar artworks of the one uploaded no matter the style of the painting.
    It will do a similarity search to see if the painting uploaded is in the database and if it is, won't return it as an output."""
    ...
    # Search if the image is matches with the first result as an output(distance of 10 was decided after a few tests)
...
async def receive_image(img: UploadFile=File(...)):
    """This function define the style of the painting. Then it will return the 5 most similar artworks with the same style of the one uploaded.
    It will do a similarity search to see if the painting uploaded is in the database and if it is, won't return it as an output."""
...
async def receive_image(img: UploadFile=File(...)):
    """This function define the style of the painting. Then it will return the 5 most similar artworks with a different style of the one uploaded.
    It will do a similarity search to see if the painting uploaded is in the database and if it is, won't return it as an output."""
...
async def receive_image(img: UploadFile=File(...)):
    """This function does a similarity search to see if the painting uploaded is in the database and if it is it will return artwork infos : artist and filename (we don't have the artwork name in this database)."""
